...
//$tables_to_search = array('patient_table');//,'table_name_2','table_name_3');
...
// is_set replacement function..
	if(isset($_REQUEST[$key]) && (!empty($_REQUEST[$key]) || $_REQUEST[$key] === '0')) {
		return true;
	}
	return false;
}
...
function db_connect ($server, $dbuser, $pass, $dbname) {
	...
	$link = @mysqli_connect($server, $dbuser, $pass);
	if (!$link) {
